...
def ProcessImg(img_name, read_path, write=True, write_path=None, res_shape=(227,227)):
    ...
    print("img sizeeeeeeeeeee:", img.size)
    img=cv2.resize(img,res_shape)
    print("img sizeeeeeeeeeeeddddddddddddd:", img.size)
...
def GlobalNormalization(img_list, name = None, batch_size=50, save_path='Train_Data', save_data=True):
    num_images = len(img_list)
    for i in range(0, num_images, batch_size):
        batch = img_list[i:i + batch_size]
        img_arr = np.array(batch)
        # Normalize
        img_arr = (img_arr - np.mean(img_arr)) / np.std(img_arr)
        # Clip negative values
        img_arr = np.clip(img_arr, 0, 1)
        # Save batch
        batch_filename = f'{save_path}/normalized_batch_{i//batch_size}.npy'
        np.save(batch_filename, img_arr)
        print(f'Batch {i//batch_size} saved successfully.')
...
def Vid2Frame(vid_path, frames_dir, ext_vid='.avi', frames_ext='.tif'):
    ...
    for vid in tqdm(vids):
        vid_path = Path(vid)
        path = vid_path.parent.as_posix() + '/'
        v_file = vid_path.name
...
def Fit_Preprocessing(path, frames_ext):
    ...
    # print(onlyfiles)
    print(len(onlyfiles),"---------", len(file_names),"-----------------", len(dirs))
    img_list = []
    for i in tqdm(range(len(onlyfiles))):
        ...
        for img in images:
            ...
            print(img_name)
            write_path = 'ProcessedImages/'+'/'.join(path.split('/')[1:]) + '/' + dirs[i]
            print(write_path)
